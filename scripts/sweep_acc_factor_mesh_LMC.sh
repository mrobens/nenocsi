...
#***************************************************************************************
#                                                                                      
# This shell script applies to the mesh toplogy and destination-address driven local   
# multicast routing. It iterates through different acceleration factors and uses the   
# command line interface of NENoCSi in order to override the default settings read
# from the configuration file config.yaml. It calls one to two additional shell scripts
# for evaluation and figure creation purposes. Look at these files for more information.
...
do
    ...
    ./nenocsi -config config.yaml -topology ${TOPOLOGY^^?} -sim $(( $SIM_TIME / $ACC ))\
	      ...
	      -size 1 16 -ofile_suffix $SUFFIX > $OFNAME 2>&1
done
