...
#**************************************************************************************
#                                                                                      
# This shell script applies to the torus toplogy and source-address driven local       
# multicast routing. It iterates through different acceleration factors and uses the  
# command line interface of NENoCSi in order to override the default settings read from
# the configuration file config.yaml. It calls one to two additional shell scripts for
...
do
    ...
    ./nenocsi -config config.yaml -topology ${TOPOLOGY^^?} -sim $(( $SIM_TIME / $ACC ))\
	      -nest_t_mul $(( $TIME_MUL / $ACC )) -casting_type ${CASTING%"_SRC"}\
	      -routing ${ROUTING} -size 1 16 -ofile_suffix $SUFFIX -nest_inp_file\
	      "../nest_inputs/data/Nest_MicroCirc_SpkTimes_NpN_256_LMC_SRC.yaml"\
	      > $OFNAME 2>&1
done
