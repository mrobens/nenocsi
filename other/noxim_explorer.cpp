 ...
 * 
 * Support for torus shaped networks was added. Also, some warnings and string
 * formatting issues were fixed.
...
#define DEF_SIMULATOR        "../bin/nenocsi"      // MR: Modified 2023/04/20
  ...
  if (pv.first == "topology")
    {
      ...
      if (width > 0 && height > 0)
        ...
        cmd = "-" + pv.first + " " + pv.second + " " +  // MR: Added linbreak and comparison to "TORUS" 2023/01/24 
	  (((pv.second == "MESH")||(pv.second == "TORUS")) ? topology_cmd.at("MESH") : topology_cmd.at("DELTA"));
    }
...
string Configuration2CmdLine(const TConfiguration& conf)
{
  ...
  cl = cl.substr(0, cl.length() - 1);                 // MR: Added 2023/01/24
		   ...
		   string& error_msg)
{
  ...
  cout << cmd << endl;
  assert(0 == system(cmd.c_str()));                   // MR: Added assert 2023/01/24
  if (!ReadResults(tmp_fname, sres, error_msg))
    return false;

  string rm_cmd = string("rm -f ") + tmp_fname;
  assert(0 == system(rm_cmd.c_str()));                // MR: Added assert 2023/01/24
...
bool RunSimulations(double start_time,
		    ...
		    string& error_msg)
{
  int    h{0}, m{0}, s{0};                      // MR: Added initialization 2023/01/24
		    ...
		    string&                    error_msg)
{
  ...
  def_cmd_line = def_cmd_line.substr(0, def_cmd_line.length() - 1);                  // MR: Added 2023/01/24
  ...
  for (uint i=0; i<conf_space.size(); i++)
    {
      ...
      string   mfname = eparams.tmp_dir + Configuration2FunctionName(conf_space[i]); // MR: Added eparams.tmp_dir 2023/01/24
