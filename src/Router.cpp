 ...
 * 
 * Support for single-flit packets as well as the torus shaped network is added.
 * In addition, some adaptations to the logging format are introduced.
      ...
      for (int j = 0; j < DIRECTIONS + 2; j++) 
	{
	  ...
	  for (int k = 0;k < GlobalParams::n_virtual_channels; k++)
	  {
		  ...
		  if (flit.flit_type == FLIT_TYPE_HEAD ||
		      flit.flit_type == FLIT_TYPE_HEADTAIL)                                                                                    // MR: Added 2022/09/18 
		    {
	  ...
	  if (reservations.size()!=0)
	  {
	     ...
	     // LOG<< "found reservation from input= " << i << "_to output= "<<o<<endl;
		  ...
		  else
		  {
		      ...
		      //LOG << " **DEBUG buffer_full_status_tx " << buffer_full_status_tx[o].read().mask[vc] << endl;                          // MR: Commented out 2022/04/01
...
{
  for (int vc=0; vc<GlobalParams::n_virtual_channels;vc++) {                                                                                   // MR: Modified 2022/03/31
    for (int i=0; i<DIRECTIONS+2; i++) {                                                                                                       // MR: Modified 2022/03/31
      buffer[i][vc].ShowStats(out);                                                                                                            // MR: Modified 2022/03/31
    }                                                                                                                                          // MR: Modified 2022/03/31
    out << endl;                                                                                                                               // MR: Modified 2022/03/31
    if (vc < GlobalParams::n_virtual_channels - 1) {                                                                                           // MR: Modified 2022/03/31
      out << setw(16) << "";                                                                                                                   // MR: Modified 2022/03/31
    }                                                                                                                                          // MR: Modified 2022/03/31
  }                                                                                                                                            // MR: Modified 2022/03/31
}
