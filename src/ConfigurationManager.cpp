 ...
 * Updated August 02, 2023 according to
 * https://github.com/davidepatti/noxim/commit/375071d77d1816420340538aad3f25ef9966d89f
 **************************************************************************************
 *
 ...
 * 
 * Extensions / modifications to account for several additional parameters.
...
void loadConfiguration() {
    ...
    //Mesh network params
    if ((GlobalParams::topology == TOPOLOGY_MESH) || (GlobalParams::topology == TOPOLOGY_TORUS)) {                                             // MR: Modified 2022/04/11
    ...
    GlobalParams::nest_input_filename = readParam<string>(config, "nest_input_filename", NEST_INPUT_FILENAME);                                 // MR: Added 2022/03/28
    GlobalParams::nest_time_multiplier = readParam<double>(config, "nest_time_multiplier", 1e9);                                               // MR: Added 2022/10/10
    GlobalParams::casting_type = readParam<CastingType>(config, "casting_type", LMC);                                                          // MR: Added 2022/03/28
    ...
    GlobalParams::create_output_files = readParam<bool>(config, "create_output_files", false);                                                 // MR: Added 2022/04/04
    GlobalParams::ofile_suffix = readParam<string>(config, "ofile_suffix", "");                                                                // MR: Added 2022/04/04
...
void showHelp(char selfname[])
{
    cout << "Usage: " << selfname << " [options]" << endl
         ...
         << "\t-nest_inp_file FILENAME\tLoad the specified NEST trace file" << endl                                                            // MR: Added 2022/03/28
         << "\t-nest_t_mul F\t\tMultiplier used to convert NEST time stamps" << endl                                                           // MR: Added 2022/10/10
         << "\t-verbose S\t\tVerbosity level of logging (if active) and global parameter display" << endl                                      // MR: Modified 2023/02/01
         << "\t\tVERBOSE_OFF\tNormal logging and aggregated statistical results" << endl                                                       // MR: Added 2023/02/01
         << "\t\tVERBOSE_MEDIUM\tAdds global parameter display and destination logging for wired connections" << endl                          // MR: Added 2023/02/01
         << "\t\tVERBOSE_HIGH\tAdds extended flit details while logging" << endl                                                               // MR: Added 2023/02/01
         ...
         << "\t-buffer_ft N\t\tSet the depth of hub buffers from tile [flits]" << endl                                                         // MR: Modified 2023/02/01
	 ...
	 << "\t-vc N\t\t\tNumber of virtual channels" << endl
         ...
         << "\t\tMESH\t\t2D Mesh" << endl
         << "\t\tTORUS\t\t2D Torus Mesh - requires XY_TORUS routing and excludes NoP selection strategy" << endl                               // MR: Added 2022/04/11
         ...
         << "\t\tXY_TORUS\tXY routing algorithm for torus network" << endl                                                                     // MR: Added 2022/04/11
         ...
         << "\t\t\t\tNOTE: If -nest_inp_file is provided, the spacial traffic distribution is automatically set to TRAFFIC_NEST_TRACE" << endl // MR: Added 2022/10/18
         << "\t-casting_type TYPE\tSet the casting type TYPE where TYPE is one of the following (only for NEST-based traffic):" << endl        // MR: Added 2022/03/28
         << "\t\tUC\t\tUnicast" << endl                                                                                                        // MR: Added 2022/03/28
         << "\t\tLMC\t\tLocal Multicast" << endl                                                                                               // MR: Added 2022/03/28
         ...
         << "\t-ofile_suffix FILENAME\tUse the given suffix for the creation of output files" << endl                                          // MR: Added 2022/04/04
         << endl
...
void showConfig()
{
    cout << "Using the following configuration: " << endl
      ...
      // << "- routing_table_filename = " << GlobalParams::routing_table_filename << endl
         ...
         << "- casting_type = " << GlobalParams::casting_type << endl                                                                          // MR: Added 2022/03/28
         ...
         << "- nest_time_multiplier = " << GlobalParams::nest_time_multiplier << endl;                                                         // MR: Added 2022/10/10
}

void checkConfiguration()
{
  if ((GlobalParams::topology==TOPOLOGY_MESH) || (GlobalParams::topology==TOPOLOGY_TORUS))                                                     // MR: Modified 2022/04/11
        {                                                                                                                                      // MR: Modified 2022/04/11
	        if (GlobalParams::mesh_dim_x <= 1) {                                                                                           // MR: Modified 2022/04/11
		        cerr << "Error: dimx must be greater than 1" << endl;                                                                  // MR: Modified 2022/04/11
		        exit(1);                                                                                                               // MR: Modified 2022/04/11
		}                                                                                                                              // MR: Modified 2022/04/11
                                                                                                                                               // MR: Modified 2022/04/11
		if (GlobalParams::mesh_dim_y <= 1) {                                                                                           // MR: Modified 2022/04/11
		        cerr << "Error: dimy must be greater than 1" << endl;                                                                  // MR: Modified 2022/04/11
		        exit(1);                                                                                                               // MR: Modified 2022/04/11
		}                                                                                                                              // MR: Modified 2022/04/11
		if (GlobalParams::winoc_dst_hops>0)                                                                                            // MR: Modified 2022/04/11
		{                                                                                                                              // MR: Modified 2022/04/11
		        cerr << "Error: winoc_dst_hops currently supported only in delta topologies" << endl;                                  // MR: Modified 2022/04/11
		        exit(1);                                                                                                               // MR: Modified 2022/04/11
		}                                                                                                                              // MR: Modified 2022/04/11
		if (GlobalParams::topology == TOPOLOGY_TORUS)                                                                                  // MR: Modified 2022/04/11
		{                                                                                                                              // MR: Modified 2022/04/11
		    if (GlobalParams::routing_algorithm.compare("XY_TORUS") != 0)                                                              // MR: Modified 2022/04/11
		    {                                                                                                                          // MR: Modified 2022/04/11
			cerr << "Error: For torus shaped mesh, only XY_TORUS routing is currently supported" << endl;                          // MR: Modified 2022/04/11
			exit(1);                                                                                                               // MR: Modified 2022/04/11
		    }                                                                                                                          // MR: Modified 2022/04/11
		    if (GlobalParams::selection_strategy.compare("NOP") == 0)                                                                  // MR: Modified 2022/04/11
		    {                                                                                                                          // MR: Modified 2022/04/11
			cerr << "Error: Torus shaped mesh currently does not support NOP selection strategy" << endl;                          // MR: Modified 2022/04/11
			exit(1);                                                                                                               // MR: Modified 2022/04/11
		    }                                                                                                                          // MR: Modified 2022/04/11
		    if (GlobalParams::n_virtual_channels != 4)                                                                                 // MR: Modified 2022/04/26
		    {                                                                                                                          // MR: Modified 2022/04/26
			cerr << "Error: Torus shaped mesh currently requires 4 virtual channels" << endl;                                      // MR: Modified 2022/04/26
			exit(1);                                                                                                               // MR: Modified 2022/04/26
		    }                                                                                                                          // MR: Modified 2022/04/26
		}                                                                                                                              // MR: Modified 2022/04/11
	}                                                                                                                                      // MR: Modified 2022/04/11
    ...
    if ((GlobalParams::min_packet_size < 2 ||                                                                                                  // MR: Modified 2022/09/22
	GlobalParams::max_packet_size < 2) &&                                                                                                  // MR: Modified 2022/09/22
        GlobalParams::traffic_distribution != TRAFFIC_NEST_TRACE) {                                                                            // MR: Modified 2022/09/22
	cerr << "Error: packet size must be >= 2" << endl;
	exit(1);
    }
    else if (GlobalParams::min_packet_size < 1 ||                                                                                              // MR: Added 2022/09/22
	     GlobalParams::max_packet_size < 1) {                                                                                              // MR: Added 2022/09/22
      cerr << "Error: packet size must be >= 1" << endl;                                                                                       // MR: Added 2022/09/22
      exit(1);                                                                                                                                 // MR: Added 2022/09/22
    }                                                                                                                                          // MR: Added 2022/09/22
    //    else if ((GlobalParams::min_packet_size == 1 &&                                                                                      // MR: No longer required 2022/09/22
    //	            GlobalParams::max_packet_size == 1) &&                                                                                     // MR: No longer required 2022/09/22
    //	           GlobalParams::use_winoc) {                                                                                                  // MR: No longer required 2022/09/22
    //      cerr << "Error: Currently, single flit packets and WiNoC can not be used simultaneously." << endl;                                 // MR: No longer required 2022/09/22
    //      exit(1);                                                                                                                           // MR: No longer required 2022/09/22
    //    }                                                                                                                                    // MR: No longer required 2022/09/22
    ...
    if (GlobalParams::casting_type < 0 ||                                                                                                      // MR: Added 2022/03/28
	GlobalParams::casting_type > 1) {                                                                                                      // MR: Added 2022/03/28
        cerr << "Error: Casting type must be in the interval [0,1]" << endl;                                                                   // MR: Added 2022/03/28
        exit(1);                                                                                                                               // MR: Added 2022/03/28
    }                                                                                                                                          // MR: Added 2022/03/28

    if (GlobalParams::nest_time_multiplier < 1)                                                                                                // MR: Added 2022/10/10
    {                                                                                                                                          // MR: Added 2022/10/10
      cerr << "ERROR: Time multiplier for NEST time stamp conversion must be larger or equal 1" << endl;                                       // MR: Added 2022/10/10
      exit(1);                                                                                                                                 // MR: Added 2022/10/10
    }                                                                                                                                          // MR: Added 2022/10/10

    for (unsigned int i = 0; i < GlobalParams::hotspots.size(); i++) {
        if ((GlobalParams::topology==TOPOLOGY_MESH) ||
	    (GlobalParams::topology==TOPOLOGY_TORUS)) {                                                                                        // MR: Modified 2022/04/11
...
void parseCmdLine(int arg_num, char *arg_vet[])
{
	...
	for (int i = 1; i < arg_num; i++) 
	{
	    if (!strcmp(arg_vet[i], "-verbose"))
	        GlobalParams::verbose_mode = arg_vet[++i];                                                                                     // MR: Modified 2023/02/01
	    ...
	    else if (!strcmp(arg_vet[i], "-casting_type")) {                                                                                   // MR: Added 2022/03/28
	      GlobalParams::casting_type = strcmp(arg_vet[++i],"UC") ? LMC : UC;                                                               // MR: Added 2022/03/28
	    }                                                                                                                                  // MR: Added 2022/03/28
	    ...
	    else if (!strcmp(arg_vet[i], "-nest_inp_file"))                                                                                    // MR: Added 2022/03/28
	    {                                                                                                                                  // MR: Added 2022/03/28
	      GlobalParams::nest_input_filename = arg_vet[++i];                                                                                // MR: Added 2022/03/28
	      GlobalParams::traffic_distribution = TRAFFIC_NEST_TRACE;                                                                         // MR: Added 2022/03/28
	    }                                                                                                                                  // MR: Added 2022/03/28
	    else if (!strcmp(arg_vet[i], "-nest_t_mul"))                                                                                       // MR: Added 2022/10/10
	      GlobalParams::nest_time_multiplier = atof(arg_vet[++i]);                                                                         // MR: Added 2022/10/10
	    ...
	    else if (!strcmp(arg_vet[i], "-ofile_suffix")) {                                                                                   // MR: Added 2022/04/04
	        GlobalParams::ofile_suffix = arg_vet[++i];                                                                                     // MR: Added 2022/04/04
	        GlobalParams::create_output_files = true;                                                                                      // MR: Added 2022/04/04
	    }                                                                                                                                  // MR: Added 2022/04/04   
...
void configure(int arg_num, char *arg_vet[]) {
    ...
    // Show configuration
    if (GlobalParams::verbose_mode != "VERBOSE_OFF")                                                                                           // MR: Modified 2022/03/31
	showConfig();
}
