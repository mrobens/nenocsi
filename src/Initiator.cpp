 ...
 * 
 * Small changes to support single-flit packets.
...
void Initiator::thread_process()
{
	...
	while (1)
	{
		...
		if (flit_payload.flit_type == FLIT_TYPE_HEAD ||
		    flit_payload.flit_type == FLIT_TYPE_HEADTAIL)                                                                              // MR: Added 2022/09/22
		{
		...
		if (!trans->is_response_error() )
		{
			...
			if (flit_payload.flit_type == FLIT_TYPE_HEAD)                                                                          // MR: Does not apply to FLIT_HEAD_TAIL
