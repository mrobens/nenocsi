 ...
 * 
 * Support for traffic generated by NEST as well as torus shaped networks is added.
 * This includes loading a YAML file by an instance of GlobalNestTrace, configuring 
 * the processing elements with this information and the provisioning of a function 
 * that creates and configures the torus shaped network.
...
void NoC::buildCommon()
{
	...
	// Check for NEST input trace availability                                                                                             // MR: Added 2022/03/28
	if (GlobalParams::traffic_distribution == TRAFFIC_NEST_TRACE)                                                                          // MR: Added 2022/03/28
	        assert(gnesttrace.load(GlobalParams::nest_input_filename));                                                                    // MR: Added 2022/03/28
...
// assert
	...
	for (int j = 0; j < dimY; j++) {
		for (int i = 0; i < dimX; i++) {
			...
			t[i][j]->pe->nest_trace = &gnesttrace;  // Needed to choose destination                                                // MR: Added 2022/03/28
	...
	for (int i = 0; i < n; i++)
	{
		...
		{                                                                                                                              // MR: Added 2022/03/28
		        core[i]->pe->nest_trace = &gnesttrace;  // Needed to choose destination                                                // MR: Added 2022/03/28
		        core[i]->pe->never_transmit = !gnesttrace.occurrencesAsSource(core[i]->pe->local_id);                                  // MR: Added 2022/03/28
		}                                                                                                                              // MR: Added 2022/03/28
		else
...
void NoC::buildMesh()
{
	...
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
		...
		{                                                                                                                              // MR: Added 2022/03/28
		        t[i][j]->pe->nest_trace = &gnesttrace; // Needed to choose destination                                                 // MR: Added 2022/03/28
		        t[i][j]->pe->never_transmit = !gnesttrace.occurrencesAsSource(t[i][j]->pe->local_id);                                  // MR: Added 2022/03/28
		}                                                                                                                              // MR: Added 2022/03/28
		else
...
void NoC::buildTorus()                                                                                                                         // MR: Added 2022/04/11
{                                                                                                                                              // MR: Added 2022/04/11
    /* Note: Routing algorithms are based on IDs / coordinates.                                                                                // MR: Added 2022/04/11
     *       So far, only XY_Torus has been adapted to support                                                                                 // MR: Added 2022/04/11
     *       torus shaped networks. In ConfigurationManager it                                                                                 // MR: Added 2022/04/11
     *       should be checked, that the correct combination                                                                                   // MR: Added 2022/04/11
     *       is used. The NOP selection algorithm also depends                                                                                 // MR: Added 2022/04/11
     *       on IDs / coordinates via the method                                                                                               // MR: Added 2022/04/11
     *       router->getNeighborId(current_id, directions[i])                                                                                  // MR: Added 2022/04/11
     *       and thus isn't supported as well                                                                                                  // MR: Added 2022/04/11
     */                                                                                                                                        // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    buildCommon();                                                                                                                             // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    // Initialize signals                                                                                                                      // MR: Added 2022/04/11
    // MR: Compared to buildMesh(), both variables are reduced                                                                                 // MR: Added 2022/04/11
    //     by one, since there is special treatment of boundary nodes                                                                          // MR: Added 2022/04/11
    int dimX = GlobalParams::mesh_dim_x;                                                                                                       // MR: Added 2022/04/11
    int dimY = GlobalParams::mesh_dim_y;                                                                                                       // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    // For most of the rest, we can procede as in buildMesh().                                                                                 // MR: Added 2022/04/11
    // Differences only occur while establishing connections                                                                                   // MR: Added 2022/04/11
    // and treating boundary nodes                                                                                                             // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    req = new sc_signal_NSWEH<bool>*[dimX];                                                                                                    // MR: Added 2022/04/11
    ack = new sc_signal_NSWEH<bool>*[dimX];                                                                                                    // MR: Added 2022/04/11
    buffer_full_status = new sc_signal_NSWEH<TBufferFullStatus>*[dimX];                                                                        // MR: Added 2022/04/11
    flit = new sc_signal_NSWEH<Flit>*[dimX];                                                                                                   // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    free_slots = new sc_signal_NSWE<int>*[dimX];                                                                                               // MR: Added 2022/04/11
    nop_data = new sc_signal_NSWE<NoP_data>*[dimX];                                                                                            // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    for (int i=0; i < dimX; i++) {                                                                                                             // MR: Added 2022/04/11
        req[i] = new sc_signal_NSWEH<bool>[dimY];                                                                                              // MR: Added 2022/04/11
        ack[i] = new sc_signal_NSWEH<bool>[dimY];                                                                                              // MR: Added 2022/04/11
	buffer_full_status[i] = new sc_signal_NSWEH<TBufferFullStatus>[dimY];                                                                  // MR: Added 2022/04/11
        flit[i] = new sc_signal_NSWEH<Flit>[dimY];                                                                                             // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
        free_slots[i] = new sc_signal_NSWE<int>[dimY];                                                                                         // MR: Added 2022/04/11
        nop_data[i] = new sc_signal_NSWE<NoP_data>[dimY];                                                                                      // MR: Added 2022/04/11
    }                                                                                                                                          // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    t = new Tile**[GlobalParams::mesh_dim_x];                                                                                                  // MR: Added 2022/04/11
    for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {                                                                                       // MR: Added 2022/04/11
    	t[i] = new Tile*[GlobalParams::mesh_dim_y];                                                                                            // MR: Added 2022/04/11
    }                                                                                                                                          // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
    // Create the mesh as a matrix of tiles                                                                                                    // MR: Added 2022/04/11
    for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {                                                                                       // MR: Added 2022/04/11
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {                                                                                   // MR: Added 2022/04/11
	    // Create the single Tile with a proper name                                                                                       // MR: Added 2022/04/11
	    char tile_name[64];                                                                                                                // MR: Added 2022/04/11
	    Coord tile_coord;                                                                                                                  // MR: Added 2022/04/11
	    tile_coord.x = i;                                                                                                                  // MR: Added 2022/04/11
	    tile_coord.y = j;                                                                                                                  // MR: Added 2022/04/11
	    int tile_id = coord2Id(tile_coord);                                                                                                // MR: Added 2022/04/11
	    sprintf(tile_name, "Tile[%02d][%02d]_(#%d)", i, j, tile_id);                                                                       // MR: Added 2022/04/11
	    t[i][j] = new Tile(tile_name, tile_id);                                                                                            // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // Tell to the router its coordinates                                                                                              // MR: Added 2022/04/11
	    t[i][j]->r->configure(j * GlobalParams::mesh_dim_x + i,                                                                            // MR: Added 2022/04/11
				  GlobalParams::stats_warm_up_time,                                                                            // MR: Added 2022/04/11
				  GlobalParams::buffer_depth,                                                                                  // MR: Added 2022/04/11
				  grtable);                                                                                                    // MR: Added 2022/04/11
	    t[i][j]->r->power.configureRouter(GlobalParams::flit_size,                                                                         // MR: Added 2022/04/11
		      			      GlobalParams::buffer_depth,                                                                      // MR: Added 2022/04/11
					      GlobalParams::flit_size,                                                                         // MR: Added 2022/04/11
					      string(GlobalParams::routing_algorithm),                                                         // MR: Added 2022/04/11
					      "default");                                                                                      // MR: Added 2022/04/11
					                                                                                                       // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // Tell to the PE its coordinates                                                                                                  // MR: Added 2022/04/11
	    t[i][j]->pe->local_id = j * GlobalParams::mesh_dim_x + i;                                                                          // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // Check for traffic table availability                                                                                            // MR: Added 2022/04/11
   		if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED)                                                                 // MR: Added 2022/04/11
		{                                                                                                                              // MR: Added 2022/04/11
			 t[i][j]->pe->traffic_table = &gttable;	// Needed to choose destination                                                // MR: Added 2022/04/11
	   		 t[i][j]->pe->never_transmit = (gttable.occurrencesAsSource(t[i][j]->pe->local_id) == 0);                              // MR: Added 2022/04/11
		}                                                                                                                              // MR: Added 2022/04/11
		else if (GlobalParams::traffic_distribution == TRAFFIC_NEST_TRACE)                                                             // MR: Added 2022/03/28
		{                                                                                                                              // MR: Added 2022/03/28
		        t[i][j]->pe->nest_trace = &gnesttrace; // Needed to choose destination                                                 // MR: Added 2022/03/28
		...
		else                                                                                                                           // MR: Added 2022/04/11
			t[i][j]->pe->never_transmit = false;                                                                                   // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // Map clock and reset                                                                                                             // MR: Added 2022/04/11
	    t[i][j]->clock(clock);                                                                                                             // MR: Added 2022/04/11
	    t[i][j]->reset(reset);                                                                                                             // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // MR: Added 2022/04/11 - Variables to cover boundary effects                                                                      // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    int ip = (i + 1) % GlobalParams::mesh_dim_x;                                                                                       // MR: Added 2022/04/11
	    int jp = (j + 1) % GlobalParams::mesh_dim_y;                                                                                       // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
            // MR: Replaced all occurences of i + 1 and j + 1 by ip and jp                                                                     // MR: Added 2022/04/11
	    // Map Rx signals                                                                                                                  // MR: Added 2022/04/11
	    t[i][j]->req_rx[DIRECTION_NORTH] (req[i][j].south);                                                                                // MR: Added 2022/04/11
	    t[i][j]->flit_rx[DIRECTION_NORTH] (flit[i][j].south);                                                                              // MR: Added 2022/04/11
	    t[i][j]->ack_rx[DIRECTION_NORTH] (ack[i][j].north);                                                                                // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_rx[DIRECTION_NORTH] (buffer_full_status[i][j].north);                                                  // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_rx[DIRECTION_EAST] (req[ip][j].west);                                                                                 // MR: Added 2022/04/11
	    t[i][j]->flit_rx[DIRECTION_EAST] (flit[ip][j].west);                                                                               // MR: Added 2022/04/11
	    t[i][j]->ack_rx[DIRECTION_EAST] (ack[ip][j].east);                                                                                 // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_rx[DIRECTION_EAST] (buffer_full_status[ip][j].east);                                                   // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_rx[DIRECTION_SOUTH] (req[i][jp].north);                                                                               // MR: Added 2022/04/11
	    t[i][j]->flit_rx[DIRECTION_SOUTH] (flit[i][jp].north);                                                                             // MR: Added 2022/04/11
	    t[i][j]->ack_rx[DIRECTION_SOUTH] (ack[i][jp].south);                                                                               // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_rx[DIRECTION_SOUTH] (buffer_full_status[i][jp].south);                                                 // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_rx[DIRECTION_WEST] (req[i][j].east);                                                                                  // MR: Added 2022/04/11
	    t[i][j]->flit_rx[DIRECTION_WEST] (flit[i][j].east);                                                                                // MR: Added 2022/04/11
	    t[i][j]->ack_rx[DIRECTION_WEST] (ack[i][j].west);                                                                                  // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_rx[DIRECTION_WEST] (buffer_full_status[i][j].west);                                                    // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // Map Tx signals                                                                                                                  // MR: Added 2022/04/11
	    t[i][j]->req_tx[DIRECTION_NORTH] (req[i][j].north);                                                                                // MR: Added 2022/04/11
	    t[i][j]->flit_tx[DIRECTION_NORTH] (flit[i][j].north);                                                                              // MR: Added 2022/04/11
	    t[i][j]->ack_tx[DIRECTION_NORTH] (ack[i][j].south);                                                                                // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_tx[DIRECTION_NORTH] (buffer_full_status[i][j].south);                                                  // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_tx[DIRECTION_EAST] (req[ip][j].east);                                                                                 // MR: Added 2022/04/11
	    t[i][j]->flit_tx[DIRECTION_EAST] (flit[ip][j].east);                                                                               // MR: Added 2022/04/11
	    t[i][j]->ack_tx[DIRECTION_EAST] (ack[ip][j].west);                                                                                 // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_tx[DIRECTION_EAST] (buffer_full_status[ip][j].west);                                                   // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_tx[DIRECTION_SOUTH] (req[i][jp].south);                                                                               // MR: Added 2022/04/11
	    t[i][j]->flit_tx[DIRECTION_SOUTH] (flit[i][jp].south);                                                                             // MR: Added 2022/04/11
	    t[i][j]->ack_tx[DIRECTION_SOUTH] (ack[i][jp].north);                                                                               // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_tx[DIRECTION_SOUTH] (buffer_full_status[i][jp].north);                                                 // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->req_tx[DIRECTION_WEST] (req[i][j].west);                                                                                  // MR: Added 2022/04/11
	    t[i][j]->flit_tx[DIRECTION_WEST] (flit[i][j].west);                                                                                // MR: Added 2022/04/11
	    t[i][j]->ack_tx[DIRECTION_WEST] (ack[i][j].east);                                                                                  // MR: Added 2022/04/11
	    t[i][j]->buffer_full_status_tx[DIRECTION_WEST] (buffer_full_status[i][j].east);                                                    // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // TODO: check if hub signal is always required                                                                                    // MR: Added 2022/04/11
	    // signals/port when tile receives(rx) from hub                                                                                    // MR: Added 2022/04/11
	    t[i][j]->hub_req_rx(req[i][j].from_hub);                                                                                           // MR: Added 2022/04/11
	    t[i][j]->hub_flit_rx(flit[i][j].from_hub);                                                                                         // MR: Added 2022/04/11
	    t[i][j]->hub_ack_rx(ack[i][j].to_hub);                                                                                             // MR: Added 2022/04/11
	    t[i][j]->hub_buffer_full_status_rx(buffer_full_status[i][j].to_hub);                                                               // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // signals/port when tile transmits(tx) to hub                                                                                     // MR: Added 2022/04/11
	    t[i][j]->hub_req_tx(req[i][j].to_hub); // 7, sc_out                                                                                // MR: Added 2022/04/11
	    t[i][j]->hub_flit_tx(flit[i][j].to_hub);                                                                                           // MR: Added 2022/04/11
	    t[i][j]->hub_ack_tx(ack[i][j].from_hub);                                                                                           // MR: Added 2022/04/11
	    t[i][j]->hub_buffer_full_status_tx(buffer_full_status[i][j].from_hub);                                                             // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
            // TODO: Review port index. Connect each Hub to all its Channels                                                                   // MR: Added 2022/04/11
            map<int, int>::iterator it = GlobalParams::hub_for_tile.find(tile_id);                                                             // MR: Added 2022/04/11
            if (it != GlobalParams::hub_for_tile.end())                                                                                        // MR: Added 2022/04/11
            {                                                                                                                                  // MR: Added 2022/04/11
                int hub_id = GlobalParams::hub_for_tile[tile_id];                                                                              // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                // The next time that the same HUB is considered, the next                                                                     // MR: Added 2022/04/11
                // port will be connected                                                                                                      // MR: Added 2022/04/11
                int port = hub_connected_ports[hub_id]++;                                                                                      // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                hub[hub_id]->tile2port_mapping[t[i][j]->local_id] = port;                                                                      // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                hub[hub_id]->req_rx[port](req[i][j].to_hub);                                                                                   // MR: Added 2022/04/11
                hub[hub_id]->flit_rx[port](flit[i][j].to_hub);                                                                                 // MR: Added 2022/04/11
                hub[hub_id]->ack_rx[port](ack[i][j].from_hub);                                                                                 // MR: Added 2022/04/11
                hub[hub_id]->buffer_full_status_rx[port](buffer_full_status[i][j].from_hub);                                                   // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
                hub[hub_id]->flit_tx[port](flit[i][j].from_hub);                                                                               // MR: Added 2022/04/11
                hub[hub_id]->req_tx[port](req[i][j].from_hub);                                                                                 // MR: Added 2022/04/11
                hub[hub_id]->ack_tx[port](ack[i][j].to_hub);                                                                                   // MR: Added 2022/04/11
                hub[hub_id]->buffer_full_status_tx[port](buffer_full_status[i][j].to_hub);                                                     // MR: Added 2022/04/11
            }                                                                                                                                  // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
            // Map buffer level signals (analogy with req_tx/rx port mapping)                                                                  // MR: Added 2022/04/11
	    // MR: Added 2022/04/11 - Replaced all occurences of i + 1 and j + 1 by ip and jp                                                  // MR: Added 2022/04/11
	    t[i][j]->free_slots[DIRECTION_NORTH] (free_slots[i][j].north);                                                                     // MR: Added 2022/04/11
	    t[i][j]->free_slots[DIRECTION_EAST] (free_slots[ip][j].east);                                                                      // MR: Added 2022/04/11
	    t[i][j]->free_slots[DIRECTION_SOUTH] (free_slots[i][jp].south);                                                                    // MR: Added 2022/04/11
	    t[i][j]->free_slots[DIRECTION_WEST] (free_slots[i][j].west);                                                                       // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->free_slots_neighbor[DIRECTION_NORTH] (free_slots[i][j].south);                                                            // MR: Added 2022/04/11
	    t[i][j]->free_slots_neighbor[DIRECTION_EAST] (free_slots[ip][j].west);                                                             // MR: Added 2022/04/11
	    t[i][j]->free_slots_neighbor[DIRECTION_SOUTH] (free_slots[i][jp].north);                                                           // MR: Added 2022/04/11
	    t[i][j]->free_slots_neighbor[DIRECTION_WEST] (free_slots[i][j].east);                                                              // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    // NoP                                                                                                                             // MR: Added 2022/04/11
	    t[i][j]->NoP_data_out[DIRECTION_NORTH] (nop_data[i][j].north);                                                                     // MR: Added 2022/04/11
	    t[i][j]->NoP_data_out[DIRECTION_EAST] (nop_data[ip][j].east);                                                                      // MR: Added 2022/04/11
	    t[i][j]->NoP_data_out[DIRECTION_SOUTH] (nop_data[i][jp].south);                                                                    // MR: Added 2022/04/11
	    t[i][j]->NoP_data_out[DIRECTION_WEST] (nop_data[i][j].west);                                                                       // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	    t[i][j]->NoP_data_in[DIRECTION_NORTH] (nop_data[i][j].south);                                                                      // MR: Added 2022/04/11
	    t[i][j]->NoP_data_in[DIRECTION_EAST] (nop_data[ip][j].west);                                                                       // MR: Added 2022/04/11
	    t[i][j]->NoP_data_in[DIRECTION_SOUTH] (nop_data[i][jp].north);                                                                     // MR: Added 2022/04/11
	    t[i][j]->NoP_data_in[DIRECTION_WEST] (nop_data[i][j].east);                                                                        // MR: Added 2022/04/11
                                                                                                                                               // MR: Added 2022/04/11
	}                                                                                                                                      // MR: Added 2022/04/11
    }                                                                                                                                          // MR: Added 2022/04/11

    /* MR: Added 2022/04/11
     *     Due to the treatment of boundary nodes as explained above, no
     *     boundary effects occur and no clearance of signals is required.
     *     Therefore, following definiton and loops are not required.
     */     
